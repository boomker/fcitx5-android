...
这里聚合最近一轮提交里最值得关注的变动，方便快速了解当前分支的演进方向：

- 候选栏交互继续增强：
  - `CandidatesView` 新增候选操作菜单支持，便于在候选项上挂接更多扩展动作。
  - 修复 API 35+ 下 `CursorAnchorInfo` 无效时的候选栏定位问题。
  - 接入硬件键盘时补充状态图标展示，外接键盘场景下的状态反馈更直观。
- 本地 AI 与预测链路继续打磨：
  - 本地 AI 新增 thinking mode，并补齐 HuggingFace 模型 ID 解析与 bundle 下载能力。
  - 远端 prompt 结构重整为 XML 分段，候选数量归一化逻辑集中收口，便于后续继续扩展。
  - 修复 think tag 的孤儿片段解析问题，并进一步压低本地建议延迟。
- 剪贴板与列表反馈体验更稳定：
  - 移除 3 秒剪贴板超时上限，改进裁剪策略，减少大条目或慢路径下的异常中断。
  - `BaseDynamicListUi` 在项目变化时会稳定弹出 Snackbar，列表类设置页的反馈更一致。
- 兼容性与代码维护同步推进：
  - 让上游 cherry-pick 过来的输入改动继续兼容当前 fork 的包结构。
  - 移除误用的 `kotlinx.coroutines.NonCancellable`，减少潜在协程语义问题。
  - 迁移掉已弃用的 `androidx.core.os.bundleOf` 用法，顺手清理一批维护性债务。
