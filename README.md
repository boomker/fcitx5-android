...
- 设置搜索体验补齐：
  - 主设置页新增搜索入口，支持跨页面跳转并自动滚动到目标设置项。
  - 搜索结果按标题和路径去重，避免同一设置项重复出现。
  - 工具栏菜单项也纳入搜索索引，常用入口更容易直接定位。
- 键盘与候选栏行为继续打磨：
  - 物理键盘候选栏开关移动到键盘设置区，并补充了输入设备相关测试。
  - 浮动键盘状态会持久保存，同时修复了拖拽手柄与按键样式细节。
  - 翻页候选列表会始终重新绑定候选视图，减少复用导致的显示错乱。
  - 圆形 Gboard 侧边按键的模糊裁剪形状得到修正。
- 工具栏与编辑器界面更清晰：
  - 多个布局、按键、字体、popup 编辑界面的保存操作改为图标按钮。
  - 保存按钮会根据可保存状态切换 tint，界面反馈更明确。
- AI 与稳定性维护：
  - AI 提供方新增 Moonshot，并调整 Zhipu 的展示顺序。
  - 修复插件服务已断开时 `onStop` 触发的崩溃。
