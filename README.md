...
- 构建系统新增 `mainline` flavor，可通过 `-PincludeMainlineFlavor=true` 生成无 `.fx` 后缀的主线命名变体，并补齐 CI 与多处应用名/包名兼容逻辑。
- 新增 Monet 主题编辑能力，支持更完整的动态配色主题编辑与管理流程。
- 文本键盘布局编辑器增强，标题显示当前编辑布局；未保存直接退出时弹出确认，降低误操作风险。
- 工具栏右侧“隐藏键盘/语音输入”按钮状态重构，抽取统一刷新逻辑，修复主题切换后语音按钮状态不一致问题。
