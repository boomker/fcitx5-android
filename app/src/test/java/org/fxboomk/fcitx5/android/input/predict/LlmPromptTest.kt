...
class LlmPromptTest {
    ...
    fun completionPromptPartsSwitchToTranslateMode() {
        ...
        assertTrue(system.contains("释义：<最常用中文释义>"))
        assertTrue(system.contains("The 音标 line must be on its own line."))
        assertTrue(system.contains("Each part of speech must be on its own separate line"))
        assertTrue(system.contains("Keep the line order fixed as: 释义 -> 音标 -> n. -> v. -> adj. -> adv. -> prep. -> phr."))
        assertTrue(system.contains("never reorder the remaining lines"))
        assertTrue(user.contains("Translate the full text below into natural Chinese"))
        assertTrue(user.contains("fixed line order"))
        assertTrue(user.contains("Skip unavailable lines but do not reorder"))
        assertTrue(!user.contains("<persona>"))
