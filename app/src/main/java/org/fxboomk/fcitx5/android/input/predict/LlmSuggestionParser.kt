...
object LlmSuggestionParser {
        ...
        raw.lineSequence()
            ...
            .let { candidates ->
                finalizeSuggestions(
                    ...
                    maxOverLimitSuggestions = if (candidates.size > 1) 2 else MAX_OVER_LIMIT_SUGGESTIONS,
                )
            }
    ...
    private fun shouldSuppressMostlyLatin(text: String, typedPrefix: String): Boolean {
        ...
        if (LlmLanguageDetector.prefersLatinSuggestions(typedPrefix)) return false
    ...
    private fun finalizeSuggestions(
        ...
        typedPrefix: String,
        maxOverLimitSuggestions: Int = MAX_OVER_LIMIT_SUGGESTIONS,
    ): List<String> {
        val policy = lengthPolicy(typedPrefix)
        ...
        return candidates.asSequence()
            ...
            .filter { candidate ->
                if (candidate.length <= policy.softLimit) {
                    true
                } else if (overLimitCount < maxOverLimitSuggestions) {
                    overLimitCount += 1
                    true
                } else {
                    false
    ...
    private fun lengthPolicy(typedPrefix: String): SuggestionLengthPolicy =
        when (LlmLanguageDetector.detect(typedPrefix)) {
            LlmLanguage.Chinese -> SuggestionLengthPolicy(softLimit = 20, hardLimit = 120)
            LlmLanguage.English -> SuggestionLengthPolicy(softLimit = 30, hardLimit = 160)
        }
    ...
    private fun normalizeContinuationPrefix(
        ...
        return when (LlmLanguageDetector.detect(typedPrefix)) {
            LlmLanguage.English -> normalizeEnglishPrefix(text, typedPrefix, hadLeadingWhitespace)
            LlmLanguage.Chinese -> normalizeChinesePrefix(text, typedPrefix)
        }
    }
