...
class LlmCatalogClient {
    ...
    suspend fun checkConnectivity(config: LlmPrefs.Config): ConnectivityResult {
        if (config.provider == LlmPrefs.Provider.MiniMax) {
            ...
            return ConnectivityResult(
                ...
                modelCount = LlmCatalogPolicy.builtInModelsForProvider(config.provider).size,
            )
        }
        ...
        if (
            ...
            LlmCatalogPolicy.shouldProbeChatEndpoint(failure.statusCode, failure.responseBody)
        ) {
            probeChatEndpoint(config)
            return ConnectivityResult(
                endpoint = config.chatEndpoint,
    ...
    suspend fun fetchModels(config: LlmPrefs.Config): List<RemoteModel> = withContext(Dispatchers.IO) {
            ...
            for ((index, endpoint) in config.modelsCompatEndpoints.withIndex()) {
                try {
                    ...
                    val canFallback = index < config.modelsCompatEndpoints.lastIndex &&
                        LlmCatalogPolicy.shouldFallbackToNextEndpoint(failure.statusCode, failure.responseBody)
                    if (!canFallback) throw failure
                }
            }
            throw (lastFailure ?: IllegalStateException("模型列表地址为空"))
        }.recoverCatching {
            LlmCatalogPolicy.recoverModelListing(
                config = config,
    ...
    private fun fetchModelsFromEndpoint(
        ...
        val connection = (URL(endpoint).openConnection() as HttpURLConnection).apply {
            ...
            when (config.compatApi) {
                LlmPrefs.CompatApi.OpenAI -> {
                    if (config.apiKey.isNotBlank()) {
                        setRequestProperty("Authorization", "Bearer ${config.apiKey}")
                    }
                }
                LlmPrefs.CompatApi.Anthropic -> {
                    if (config.apiKey.isNotBlank()) {
    ...
    private fun probeChatEndpoint(config: LlmPrefs.Config) {
        if (config.chatEndpoint.isBlank()) error("聊天接口地址为空")
        val probeModel = LlmCatalogPolicy.connectivityProbeModel(config)
        ...
        for ((index, variant) in variants.withIndex()) {
            ...
            connection.outputStream.use { output ->
                output.write(
                    LlmRequestPolicy.applyAugmentation(
                        config = config,
                        payload = payload,
