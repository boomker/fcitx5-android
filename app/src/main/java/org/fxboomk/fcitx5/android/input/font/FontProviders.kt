...
object FontProviders {
    ...
    /**
     * Check if a specific font key is explicitly configured (not using fallback).
     * Returns true only if the key exists in fontTypefaceMap with a non-null value.
     */
    fun hasFont(key: String): Boolean {
        return fontTypefaceMap[key] != null
    }
