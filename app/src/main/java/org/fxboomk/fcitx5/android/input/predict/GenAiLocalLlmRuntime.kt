...
internal object GenAiLocalLlmRuntime : LocalLlmRuntime {
    ...
    fun smoke(request: LocalLlmPredictionRequest): SmokeResult {
        if (!isAvailable()) {
            return SmokeResult(
                stage = "unavailable",
                prompt = "",
        ...
        return runCatching {
            ...
            SmokeResult(
                ...
                suggestions = LlmSuggestionParser.parse(
    ...
    private fun configureSearch(
        ...
        request: LocalLlmPredictionRequest,
        inputTokenCount: Int,
    ) {
    ...
    private fun buildPrompt(request: LocalLlmPredictionRequest): String =
        LlmPrompt.localOnDevicePrompt(
    ...
    private fun bundleKey(request: LocalLlmPredictionRequest): String {
...
internal fun formatLocalGenerationTelemetry(telemetry: GenAiLocalLlmRuntime.LocalGenerationTelemetry): String =
...
internal fun localGenerationStopReason(
    ...
    if (request.outputMode != LlmOutputMode.Suggestions) {
        return null
    }
    return when (request.taskMode) {
        LlmTaskMode.Completion -> {
            val suggestions = LlmSuggestionParser.parse(generatedText, request.beforeCursor)
            when {
